...
const int mcs = 10000; // measurement
const int runs = 15; // number of independent runs

// output
const int binSize = 10000;
std::string DATADIR = "dataf/";
...
void MCSweep(double &E,double &M, double S[L][L], double T)
{
	for(int n=0; n<N; n++)
	{
		...
		double delE = 2*S[rx][ry]*(
				       S[(rx+1)%L][ry] 		* S[rx][(ry+1)%L] 		* S[(rx+1)%L][(ry+1)%L]
				     + S[(rx+1)%L][ry] 		* S[rx][(ry+L-1)%L] 	* S[(rx+1)%L][(ry+L-1)%L]
				     + S[(rx+L-1)%L][ry] 	* S[rx][(ry+1)%L]    	* S[(rx+L-1)%L][(ry+1)%L]
				     + S[(rx+L-1)%L][ry] 	* S[rx][(ry+L-1)%L]  	* S[(rx+L-1)%L][(ry+L-1)%L]
				);
		if(delE < 0)
		{
			S[rx][ry] = -S[rx][ry]; 
...
void Metropolis(double &E,double &M, double S[L][L], double T)
{
	//------ Equilibration ------//
